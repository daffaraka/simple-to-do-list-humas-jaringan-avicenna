    ...
    return (
      <div className="grid grid-cols-7 border-b border-borderBase">
        {daysOfWeek.map((dayName, idx) => (
          <div key={idx} className="py-2 text-center text-xs font-semibold text-textSecondary uppercase tracking-wider">
            {dayName}
          </div>
        ))}
      </div>
        ...
        >
          ...
          <div className="flex flex-col gap-1 mt-1">
            {dayCards.map(card => {
              ...
              const initials = picObj ? picObj.name.charAt(0).toUpperCase() : (typeof card.pic === 'string' ? card.pic.charAt(0).toUpperCase() : '');
              const displayText = initials ? `(${initials}) ${card.title}` : card.title;

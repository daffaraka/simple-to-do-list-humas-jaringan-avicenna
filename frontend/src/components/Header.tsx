...
import { Search, Tag, Users, Network, LayoutGrid, Calendar as CalendarIcon, LogOut, Sun, Moon, Layers } from 'lucide-react';
import { AVAILABLE_LABELS } from '../types';
import { useKanban } from '../store/kanbanStore';
import { useAuthStore } from '../store/authStore';

export function Header() {
  const { searchQuery, setSearchQuery, filterLabel, setFilterLabel, activeDepartment, setActiveDepartment, viewMode, setViewMode, isDarkMode, toggleDarkMode } = useKanban();
        ...
        <div className="hidden md:flex bg-black/40 p-1 rounded-lg border border-white/5 mx-auto">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-medium transition-all ${
              viewMode === 'kanban' 
                ? 'bg-textSecondary/20 text-textPrimary shadow-sm' 
